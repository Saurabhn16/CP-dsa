...
void solve(){
    ...
    //dp[i][j] is the max value we can get if we have used first i things and have utilzed wt of j
    vector<vector<ll>> dp(n+4,vector<ll> (w+3,0));
    dp[0][wt[0]]=v[0];
    fo(i,1,n+1){
        rep(j,w+1){
        	dp[i][j]=dp[i-1][j];

            if(j-wt[i-1]>=0)dp[i][j]=max(dp[i][j],dp[i-1][j-wt[i-1]]+v[i-1]);
        }
    }
