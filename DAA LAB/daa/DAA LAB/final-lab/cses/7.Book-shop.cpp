...
void solve(){
    ...
    vi noofpages(n);
    rep(i,n)cin>>price[i];
    rep(i,n)cin>>noofpages[i];
    // dp[i][j] is the max no of pages we can buy in the first i books using atmost j amount
    ...
    fo(i,1,n+1){
        rep(j,x+1){
            dp[i][j]=dp[i-1][j];
            if(j-price[i-1]>=0){
                dp[i][j]=max(dp[i][j],dp[i-1][j-price[i-1]]+noofpages[i-1]);
            }
        }
    }
    cout<<dp[n][x]<<endl;
