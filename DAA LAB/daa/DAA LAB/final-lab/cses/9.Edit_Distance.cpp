...
void solve(){
    ...
    cin>>a>>b;
    int n=max(a.sz,b.sz);
    //we need to make string a to string b in min no of steps and each can be three types as disccussed
    //dp[i][j] is the min no of operations if we are on the i-1th index in a and j-1th index in b
    ...
    rep(i,a.sz+1){
        rep(j,b.sz+1){
            ...
            if(i>0 and j>0 ) dp[i][j]=min(dp[i][j],dp[i-1][j-1]+(a[i-1]!=b[j-1]));
        }
    }
